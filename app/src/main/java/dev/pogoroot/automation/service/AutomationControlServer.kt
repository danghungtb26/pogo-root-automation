...
class AutomationControlServer(
    ...
    private fun applyParams(config: HeadlessAutomationConfig, params: Map<String, String>): HeadlessAutomationConfig = config.copy(
        ...
        catchAll = params.boolean("catchAll") ?: config.catchAll,
        catchThrowQuality = params["throwQuality"]?.let(::parseThrowQuality)
            ?: params.boolean("autoExcellent")?.let {
                if (it) dev.pogoroot.automation.core.automation.ThrowQualityTarget.EXCELLENT
                else dev.pogoroot.automation.core.automation.ThrowQualityTarget.ANY
            }
